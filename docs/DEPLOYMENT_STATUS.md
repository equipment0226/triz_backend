...
- 로컬 백엔드 회귀 검사 66개·브라우저 검사 6개 통과, 프런트 프로덕션 빌드 통과. 실제 데이터 검증은 미완료다.
...
- 운영 사전 점검 결과는 `NOT_CONFIGURED`다. 실제 BigQuery 조회 작업은 실행하지 않았으며, 예상 스캔량·특허 검색 결과·서비스 계정 권한은 인증 등록 후 검증해야 한다.
- 현재 운영에는 `BIGQUERY_PROJECT_ID`와 `GOOGLE_SERVICE_ACCOUNT_JSON`이 등록되지 않았다. 코드 배포 후에도 인증·dry run·샘플 조회가 확인되기 전에는 기존 공급자 선택을 유지한다.
