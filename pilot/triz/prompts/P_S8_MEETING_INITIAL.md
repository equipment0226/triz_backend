...
- rationale: 직무 관점의 구체적 근거와 성립 조건을 짧은 1문장(권장 100자 이내)으로 적는다. 비용·기간 수치는 근거와 단위를 갖춘 추정만 허용한다.
- red_flags: 위험 또는 제약 위반 의심. 확인된 제약과 연결되면 "제약위반: [제약id]"를 명시한다.
- improvement_suggestion: 점수를 1점 올리는 데 필요한 변경 또는 검증을 1문장으로 적는다.

이번 직군의 평가 기준 (이 목록의 차원만 출력):
{{dimension_rubric}}

담당 차원 {{dimensions}} 밖의 점수는 출력하지 않는다. 다른 직군의 점수를 대신 채우지 않는다.

# 질문
- 각 question은 핵심 쟁점 하나를 묻는 1문장, 120자 이내를 목표로 한다. 배경 재설명·항목 나열·서론은 생략한다.
...
[출력 JSON만]
반드시 최상위 객체에 scores와 questions를 모두 넣는다. 점수 표·코드블록·추가 설명으로 바꾸지 않는다.
들여쓰기·반복 서론을 생략한 JSON으로 출력하되 모든 평가 행과 필수 필드를 유지한다. red_flags는 위험이 없으면 []다.
