...
2026-09-16 10:39 KST 운영 반영: 직접검토 두 묶음의 출처 갱신을 AX V3 백엔드와 함께 배포했다. 정본 19개 기능군·269개, 출처 연결 247개를 유지한다. 아래 이전 검토의 로컬 발행 기록은 당시 상태이며 최신 배포 상태는 [AX V3 배포 기록](TRIZ_AX_V3_DEPLOYMENT_20260916.md)을 따른다. 직접검토 및 외부 LLM 추출 금지 정책을 유지한다.
...
대상은 DB에 적재되는 **전체 산업의 전체 특허**다. IT·통신·소프트웨어·보안·정보처리도 포함하며 업종 필터를 걸지 않는다. `effect_source_pages.harvest_page`는 검색 상위 결과가 아닌 공개번호 순서로 저장 초록을 읽는다. 2026-09-12 06:32:55 UTC 조회 당시 저장소는 5,148,204건이었다. 계속 적재 중이므로 고정 총량이 아니다.

2026-09-16 직접 검토를 재개하여 AP-00143-S1부터 AR-032666-A4까지 누적 1,480건을 조회했다. 초록 1,032건을 직접 읽어 원리 연결 440건, 추가 확인 보류 342건, 설계 구성 247건, 근거 부족 주장 제외 3건으로 기록했다. 나머지 448건은 초록 없음이며 읽거나 분석한 초록 수에 넣지 않는다. DB에서 확보한 것은 서지와 초록이며 청구항·명세서·도면 전문은 아니다. 전체 수백만 건을 읽었다는 뜻이 아니며 전체 순회는 미완료다. 이번 추가 40건의 판단과 운영 상태는 [재개 기록](RAILWAY_RESUME_20260916.md)을 참고한다.

현재 정본은 **19개 기능군, 269개 효과**다. 217개에서 52개를 직접 편집해 추가했다. IT의 OFDM·시분할 다중화·분산 장벽 동기화·지도 학습·상관 검출·변환 영역 합성곱·힐베르트 변환·최단 경로 탐색도 포함한다. 247개는 참고 문헌이 연결되고 22개는 연결 문헌 없는 편집 지식이다. 다섯 번째 검토에서는 기존 원리에 특허 근거를 추가했으며, 신규 메커니즘 후보는 교차 확인 전까지 정본에 넣지 않았다. 참고 문헌 연결이 모든 현장 조건·성능의 실증을 뜻하지 않는다. 다섯 번째 묶음은 로컬에 발행했으며 실행 중인 사용자 분석이 끝난 뒤 운영 반영할 수 있도록 보존했다.
...
원시 페이지는 `pilot/data/patent_effects_manual/page-000001.json`부터 저장돼 있다. 요약은 `pilot/research/effects/manual-progress.json`, 1,480건 상태 원장은 `pilot/data/patent_effects_manual/review-ledger.json`이다. `manual_reviews/`의 검토 기록은 각 결정을 입력 내용 해시에 결합한다. 최신 판단 TSV는 `manual-patents-2026-09-16-02.tsv`(40건)이며 불변 검토는 `manual_reviews/2026-09-16-02.json`이다. 바뀐 초록에 이전 결정을 자동 적용하지 않는다. 이번 원리 연결은 연구용 출처 원장과 로컬 serving JSON에 반영했다. 19개 기능군·269개 정본 효과를 유지하며 연결 출처를 보강했다. 외부 LLM 추출은 0회다.

아홉 페이지는 `manual_archives/2026-09-16-02.json.gz`에도 보존했다. 9월 12일의 01~05 압축본은 당시 기록으로 유지한다. 각 압축본의 내용은 공개 서지·초록 페이지 JSON의 배열이며 서로 중복되므로 합산하지 않는다. 원시 페이지가 없으면 최신 압축본에서 각각 `page-000001.json`부터 복원할 수 있다. 사용자 프로젝트·개인 첨부·인증정보는 포함하지 않는다.
...
미검토 초록이 0건일 때 다음 묶음을 조회한다. 위 명령이 다음 요청 파일을 생성한다. 현재 커서는 `AR-032666-A4`, 이번 순회의 상한은 `ZA-F202500713-S`다. 기존 파일을 덮어쓰지 않고 다음 번호로 저장한다. 서버 접속은 기존 Railway 인증을 사용하며 DB는 읽기만 한다.

```powershell
.venv/Scripts/python.exe deploy/patent_remote.py deploy/harvest_manual_patent_page.py --params pilot/data/patent_effects_manual/next-request.json --output pilot/data/patent_effects_manual/page-000010.json
```
