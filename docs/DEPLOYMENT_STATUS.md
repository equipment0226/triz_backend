...
> 아래는 이전 배포 기록이다. 2026-09-09 운영 백엔드는 `PATENT_SEARCH_PROVIDER=vector`로 전환했고, `triz-patent-vectors` 및 별도 `triz-patent-indexer` 작업 서비스를 생성했다. 초기 전체 적재는 진행 중이다. [현재 저장소 운영 가이드](PATENT_VECTOR_STORAGE.md)를 참고한다.
