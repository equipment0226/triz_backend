...
원문과 이전 답변을 긴 문장으로 반복하지 말고, 문자열은 핵심 사실·조건을 중심으로 짧게 쓴다. 중복 제약은 합치되 서로 다른 수치·단위·금지사항은 모두 보존한다. rationale은 1개 짧은 구절, candidate_characteristics와 candidate_conflicts는 각 1문장이다. JSON 밖 해설과 들여쓰기는 생략한다.
